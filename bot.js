...
const {
  ...
  makeRoleName, makeWinnerRoleName, makeItemRoleName,
} = require('./data/index');
...
// ── Inventory helpers (Curator story only) ──────────────────────
// Items are stored as hidden, mentionable=false roles named
// item-<storyId>-<key>. A player "carries" an item iff they hold its role.

/** Does the member hold the given story item? */
function hasItem(member, storyId, key) {
  const role = member.guild.roles.cache.find(r => r.name === makeItemRoleName(storyId, key));
  return !!role && member.roles.cache.has(role.id);
}

/** Every item key the member is carrying for a story (in registry order). */
function carriedItems(member, story) {
  if (!story.items) return [];
  return Object.keys(story.items).filter(key => hasItem(member, story.id, key));
}

/** All item keys takeable in the player's CURRENT (or earlier) levels of a story. */
function availableItemKeys(story, currentLevelId) {
  const keys = [];
  for (const level of story.levels) {
    if (level.id <= currentLevelId && Array.isArray(level.gives)) keys.push(...level.gives);
  }
  return keys;
}
...
async function runSetup(guild) {
  ...
  for (const story of allStories()) {
    ...
    // Hidden inventory-item roles (stories that declare an `items` registry).
    if (story.items) {
      for (const key of Object.keys(story.items)) {
        const itemRole = await getOrCreateRole(guild, makeItemRoleName(story.id, key), '#2b2d31');
        // Keep item roles invisible: not hoisted, not mentionable.
        if (itemRole.hoist || itemRole.mentionable) {
          await itemRole.edit({ hoist: false, mentionable: false }).catch(() => {});
        }
      }
    }
...
async function runTeardown(guild) {
  ...
  for (const [, role] of guild.roles.cache) {
    ...
    const isItemRole    = /^item-/.test(n);
    const isGateRole    = n === CONFIG.MEMBER_ROLE || n === CONFIG.BUMPER_ROLE;
    if (isLevelRole || isWinnerRole || isItemRole || isGateRole) {
...
function buildCommands() {
  ...
  return [
    ...
    new SlashCommandBuilder()
      .setName('take')
      .setDescription('Pick up an item you found in a room (inventory stories)')
      .addStringOption(o => o.setName('item').setDescription('Which item to pick up (e.g. lantern)').setRequired(true))
      .addStringOption(o => o
        .setName('story')
        .setDescription('For which story? (optional if you only have one active)')
        .setRequired(false)
    ...
    new SlashCommandBuilder()
      .setName('inventory')
      .setDescription('See what you\'re carrying (inventory stories)')
      .addStringOption(o => o
        .setName('story')
        .setDescription('For which story? (optional if you only have one active)')
        .setRequired(false)
...
async function handleCommand(interaction) {
  ...
  // /take and /inventory must be used inside a story's level channels
  // (the wing/room categories named "… — L3"). Players only have command
  // access there anyway, but this gives a clearer error elsewhere.
  if (commandName === 'take' || commandName === 'inventory') {
    const parentName = channel?.parent?.name ?? '';
    if (!/ — L\d+$/.test(parentName)) {
      return interaction.reply({
        content: '❌ Use `/take` and `/inventory` inside your current story rooms.',
        ephemeral: true,
      });
    }
  }
  ...
  if (commandName === 'answer') {
    ...
    // Inventory gate: even a correct answer is blocked until the player is
    // carrying every required item for this level (Curator story only).
    if (isCorrect && Array.isArray(level.requires) && level.requires.length) {
      const missing = level.requires.filter(key => !hasItem(member, story.id, key));
      if (missing.length) {
        const names = missing.map(key => {
          const it = story.items?.[key];
          return it ? `${it.emoji} ${it.name}` : key;
        });
        const embed = new EmbedBuilder()
          .setColor('#faa61a')
          .setTitle('🎒 You\'re missing something')
          .setDescription(
            `You've got the right idea, but you can't do this empty-handed.\n\n` +
            `You still need: **${names.join(', ')}**.\n\n` +
            `Find it in an earlier room and pick it up with \`/take\`, then try your answer again.`);
        return interaction.reply({ embeds: [embed], ephemeral: true });
      }
    }
  ...
  // ── /take ────────────────────────────────────────────────────
  if (commandName === 'take') {
    const rawKey = interaction.options.getString('item').toLowerCase().trim();
    const storyOpt = interaction.options.getString('story');

    // Resolve which active story this applies to (must have an inventory).
    const progress = getUserProgress(member).filter(p => p.story.items);
    if (progress.length === 0) {
      return interaction.reply({ content: '❌ You\'re not in a story with an inventory right now.', ephemeral: true });
    }

    let active;
    if (storyOpt) {
      active = progress.find(p => p.story.id === storyOpt);
      if (!active) return interaction.reply({ content: '❌ You\'re not active in that story (or it has no items).', ephemeral: true });
    } else if (progress.length === 1) {
      active = progress[0];
    } else {
      const list = progress.map(p => `\`${p.story.id}\` — ${p.story.emoji} ${p.story.name}`).join('\n');
      return interaction.reply({ content: `Several active stories with items:\n${list}\n\nExample: \`/take item:lantern story:curator\``, ephemeral: true });
    }

    const { story, level } = active;
    // Accept the item key, or match on the item's display name (spaces → dashes).
    const allKeys = Object.keys(story.items);
    let key = allKeys.find(k => k === rawKey);
    if (!key) {
      const norm = rawKey.replace(/\s+/g, '-');
      key = allKeys.find(k => k === norm
        || story.items[k].name.toLowerCase() === rawKey
        || story.items[k].name.toLowerCase().replace(/\s+/g, '-') === norm);
    }

    if (!key) {
      return interaction.reply({ content: `❌ There's no \`${rawKey}\` to take here. Read the room and check the exact name.`, ephemeral: true });
    }

    // You can only pick up items present in rooms you've reached.
    if (!availableItemKeys(story, level.id).includes(key)) {
      return interaction.reply({ content: `❌ You haven't found the **${story.items[key].emoji} ${story.items[key].name}** yet — it's in a room you haven't reached.`, ephemeral: true });
    }

    const item = story.items[key];
    if (hasItem(member, story.id, key)) {
      return interaction.reply({ content: `🎒 You're already carrying the **${item.emoji} ${item.name}**.`, ephemeral: true });
    }

    const itemRole = guild.roles.cache.find(r => r.name === makeItemRoleName(story.id, key));
    if (!itemRole) {
      return interaction.reply({ content: '⚠️ That item isn\'t set up on the server yet. Ask an admin to run `/setup`.', ephemeral: true });
    }
    await member.roles.add(itemRole).catch(console.error);

    const embed = new EmbedBuilder()
      .setColor(story.color ?? '#5865f2')
      .setTitle(`${item.emoji} Picked up: ${item.name}`)
      .setDescription(`Added to your bag. Check everything you're carrying with \`/inventory\`.`);
    await log(guild, `🎒 **${member.user.tag}** took ${item.name} in ${story.name}`);
    return interaction.reply({ embeds: [embed], ephemeral: true });
  }
  ...
  if (commandName === 'inventory') {
    ...
    const progress = getUserProgress(member).filter(p => p.story.items);
    if (progress.length === 0) {
      return interaction.reply({ content: '🎒 Your bag is empty — you\'re not in a story with items right now.', ephemeral: true });
    }
    ...
    if (storyOpt) {
      ...
      const list = progress.map(p => `\`${p.story.id}\` — ${p.story.emoji} ${p.story.name}`).join('\n');
      return interaction.reply({ content: `Several active stories with items:\n${list}\n\nExample: \`/inventory story:curator\``, ephemeral: true });
    }

    const { story } = active;
    const carried = carriedItems(member, story);
    const embed = new EmbedBuilder()
      .setColor(story.color ?? '#5865f2')
      .setTitle(`🎒 Your bag — ${story.emoji} ${story.name}`)
      .setDescription(carried.length
        ? carried.map(k => `${story.items[k].emoji} **${story.items[k].name}**`).join('\n')
        : '*Empty. Explore the rooms and use `/take` to pick things up.*');
    return interaction.reply({ embeds: [embed], ephemeral: true });
  }
  ...
  if (commandName === 'reset') {
    ...
    for (const story of stories) {
      ...
      // Drop any inventory items the player was carrying for this story.
      if (story.items) {
        for (const key of Object.keys(story.items)) {
          const itemRole = guild.roles.cache.find(r => r.name === makeItemRoleName(story.id, key));
          if (itemRole) await target.roles.remove(itemRole).catch(() => {});
        }
      }
    }
